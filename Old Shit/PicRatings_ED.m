function PicRatings_ED(varargin)
% Rate all images, choose top X picsn    

global wRect w XCENTER rects mids COLORS KEYS PicRating_ED
...
PICS =struct;
    ...
    %XXX: UPDATE THESE WITH ACTUAL NAMES OF FILES....
    PICS.in.Un = dir('Un*');
    PICS.in.H = dir('Health*');
    PICS.in.Avg = dir('Avg*');
    PICS.in.Thin = dir('Thin*');
    
    if isempty(PICS.in.Un || PICS.in.H || PICS.in.Avg || PICS.in.Thin);
        error('Could not find pics! Make sure a folder exists called "Pics" with all the appropriate images contained therein.')
    end
    
    picnames_food = {PICS.in.Un.name PICS.in.H.name; zeros(1,length(PICS.in.Un)) ones(1,length(PICS.in.H))}';
    picnames_food = picnames(randperm(size(picnames_food,1)),:);
    
    picnames_mods = {PICS.in.Thin.name PICS.in.Avg.name}';
    picnames_mods = picnames(randperm(size(picnames_mods,1)),:);
    
%     If food1st = 1, present food images first...doih.
    food1st = CoinFlip(1,.5);
    if food1st == 1;
        picnames = {picnames_food; picnames_mods};
    else
        picnames = {picnames_mods; picnames_food};
    end


jitter = BalanceTrials(length(picnames),1,[1 2 3]);

PicRating_ED = struct('filename',picnames,'Rate_App',0,'Jitter',[],'FixOnset',[],'PicOnset',[],'RatingOnset',[],'RT',[]); %,'Rate_Crave',0);

for hhh = 1:length(PicRating_ED)f;
    
    PicRating_ED(hhh).Jitter = jitter(hhh);
end
...
for x = 1:20:length(PicRating_ED);  %UPDATE TO LENGTH OF GO PICS
    for y = 1:19;
        xy = x+y;
        if xy > length(PicRating_ED)
            break
        end
        
        DrawFormattedText(w,'+','center','center',COLORS.WHITE);
        fixon = Screen('Flip',w);
%         PicRating_ED(xy).FixOnset = fixon - scan_sec;
        WaitSecs(PicRating_ED(xy).Jitter);
        
        tp = imread(getfield(PicRating_ED,{xy},'filename'));
        tpx = Screen('MakeTexture',w,tp);          
        Screen('DrawTexture',w,tpx);
        picon = Screen('Flip',w);
%         PicRating_ED(xy).PicOnset = picon - scan_sec;
        WaitSecs(trialdur);
...
%         PicRating_ED(xy).RatingOnset = rateon - scan_sec;
            
        FlushEvents();
            while 1
                [keyisdown, rt, keycode] = KbCheck();
                if (keyisdown==1 && any(keycode(KEYS.all)))
                    PicRating_ED(xy).RT = rt - rateon;
...
%             if q == 1;
           ...
           PicRating_ED(xy).Rate_App = rating;
           Screen('Flip',w);
           FlushEvents();
           WaitSecs(.25);
    end
...
presort = struct2cell(PicRating_ED)';
postsort = sortrows(presort,-2);    %Sort descending by column 2
PicRating_ED = cell2struct(postsort,fields,2);
