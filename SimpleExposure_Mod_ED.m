 function SimpleExposure_Mod_ED(varargin)
...
[mdir,~,~] = fileparts(which('SimpleExposure_Mod_ED.m'));
...
filen = sprintf('PicRate_Mod%d.mat',ID);
try
    p = open(filen);
catch
    ...
    if randopics == 1
        cd(imgdir)
        p = struct;
        p.PicRating_Mod.Avg = dir('Av*'); 
        p.PicRating_Mod.Thin = dir('Th*');
...
PICS.in.hi = struct('name',{p.PicRating_Mod.Avg(1:30).name}');
PICS.in.lo = struct('name',{p.PicRating_Mod.Thin(1:30).name}');
% neutpics = dir('water*');
...
%1 = Avg, 0 = Thin
pictype = [ones(30,1); zeros(30,1)];

%Make long list of randomized #s to represent each pic
piclist = [randperm(30)'; randperm(30)'];
...
verbage = 'How attractive do you find this person?';
